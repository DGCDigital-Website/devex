...
import { updateInvoice } from "@/lib/beacon/actions";
import { INVOICE_STATUSES, DGC_SERVICES, COUNTRIES } from "@/lib/beacon/types";
import type { BeaconUser, InvoiceRow, LineItem } from "@/lib/beacon/types";
import { FileText, User, Package, Loader2, ArrowLeft, AlertCircle, Plus, Trash2 } from "lucide-react";
...
export default function EditInvoiceForm({ user, invoice }: { user: BeaconUser; invoice: InvoiceRow }) {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>(
    (invoice.line_items && invoice.line_items.length > 0)
      ? invoice.line_items
      : [{ id: "1", item: invoice.service, description: "", cost: invoice.subtotal ?? invoice.total, qty: 1 }]
  );
  ...
  async function handleSubmit(e:React.FormEvent){
    ...
    const result = await updateInvoice(invoice.id,{
      ...
      subtotal,tax_amount:taxAmt,total,balance:String(total),
  ...
  return (
    <BeaconShell user={user} title="Edit Invoice">
      <div className="max-w-3xl">
        <Link href="/beacon/invoices" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition-colors mb-6"><ArrowLeft className="w-4 h-4"/>Back to Invoices</Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-gray-400 bg-gray-100 px-2.5 py-1 rounded-lg">{invoice.id}</span>
            </div>
            <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Field label="Due Date" required><input type="date" required value={form.due_date} onChange={(e)=>update("due_date",e.target.value)} className={INPUT_CLS}/></Field>
              <Field label="Status"><select value={form.invoice_status} onChange={(e)=>update("invoice_status",e.target.value)} className={SELECT_CLS}>{INVOICE_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}</select></Field>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
            ...
            <div className="px-6 py-5 border-t border-gray-100">
              <div className="ml-auto max-w-xs space-y-2.5">
                ...
                <div className="flex justify-between text-base font-bold text-gray-900 pt-2 border-t border-gray-200"><span>Total Due</span><span className="tabular-nums text-dgc-blue-1">{fmtKES(total)}</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pb-4">
            <button type="submit" disabled={loading} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white shadow-[0_2px_8px_rgba(61,157,217,0.3)] disabled:opacity-60 disabled:shadow-none transition-all" style={{background:"linear-gradient(135deg,#3D9DD9,#177DA6)"}}>
              {loading?<><Loader2 className="w-4 h-4 animate-spin"/>Saving…</>:"Update Invoice"}
            </button>
