...
import { updateAdminQuotation } from "@/app/admin/_actions";
...
export default function EditQuotationForm({ quotation }: { quotation: Quotation }) {
  ...
  const [client, setClient] = useState(quotation.client);
  const [email, setEmail] = useState(quotation.clientEmail);
  const [organisation, setOrganisation] = useState(quotation.clientOrg);
  const [country, setCountry] = useState(quotation.country);
  const [validUntil, setValidUntil] = useState(quotation.validUntil);
  const [status, setStatus] = useState<Quotation["status"]>(quotation.status);
  const [notes, setNotes] = useState(quotation.notes ?? "");
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  const [items, setItems] = useState<LineItem[]>(
    quotation.items.map((it) => ({ description: it.description, quantity: it.quantity, rate: it.rate }))
  );
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const result = await updateAdminQuotation(quotation.id, {
      client,
      clientEmail: email,
    ...
    setSaved(true);
    setTimeout(() => { setSaved(false); router.push(`/admin/quotations/${quotation.id}`); }, 1500);
  };

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/admin/quotations/${quotation.id}`} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-gray-700 transition-colors">
          <ArrowLeft size={18} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Quotation</h1>
          <p className="text-sm text-gray-500 mt-0.5">{quotation.quotationNo}</p>
        </div>
      </div>
      ...
      {saved && (
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl">
          <CheckCircle size={18} />
          <span className="text-sm font-medium">Quotation updated. Redirecting…</span>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" required value={validUntil} onChange={(e) => setValidUntil(e.target.value)} className={INP} />
            </div>
            <div>
